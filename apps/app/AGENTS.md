...
| `/verify-email`                                          | VerifyEmail       | `views/auth/VerifyEmailView.vue`                                                    | `requiresGuest: false` | Working |
...
| `/invitations`                                           | MyInvitations     | `views/invitations/MyInvitationsView.vue`                                           | `requiresAuth`  | Partial |
...
- **Token refresh flow**: On 401 responses:
  1. Concurrent 401s all await a single in-flight refresh promise (shared `refreshPromise`, cleared once it settles) to prevent refresh race conditions
  2. Sends refresh request (cookie-based) to `POST /auth/refresh`
  3. On success: server rotates and sets new httpOnly cookies, each 401'd request independently replays once
...
| ----------------------- | ------------------------- | ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------- |
| `useAuthStore`          | `stores/auth.js`          | `user`, `loading`                                                                        | `initAuth`, `signup`, `signin`, `logout`, `forgotPassword`, `resetPassword`                                 | Working |
...
| `useMembersStore`       | `stores/members.js`       | `members`, `loading`                                                                     | `fetchMembers`, `inviteMember`, role update/remove                                                          | Working |
| `useInvitationsStore`   | `stores/invitations.js`   | `myInvitations`, `loading`                                                               | `fetchMyInvitations` (no-op stub; backend not yet wired), `inviteToWorkspace`, `acceptInvitation`, `declineInvitation` (no-op stub) | Partial |
| `useAgentsStore`        | `stores/agents.js`        | `agents`, `loading`                                                                      | `fetchAgents`, `createAgent`, `updateAgent`, `deleteAgent`, `setDefaultAgent`                               | Working |
...
| `useWorkspacesStore`    | `stores/workspaces.js`    | `workspaces`, `currentWorkspace`, `currentPermissions`, `loading`                        | `fetchWorkspaces` (returns boolean), `fetchWorkspaceById`, `createWorkspace`, `updateWorkspace`, `deleteWorkspace`          | Working |
...
Each store also exposes a `reset()` action that restores its initial state. `stores/reset.js` is not a store but a coordinator that exports `resetAllStores()`, which calls `.reset()` on every non-auth store — invoked on logout so a subsequent login (without a hard reload) never shows the previous user's data.
...
| `useInvitations`   | `composables/useInvitations.js`   | `myInvitations`, `loading`, invite modal state, invite/accept/decline                                                                                                                                                                                                                                                                 |
...
| `useAgents`        | `composables/useAgents.js`        | `agents`, `loading`, `viewMode`, `sortBy`, `sortOrder`, modal state (`drawerAgent`, `isEditing`), CRUD wrappers, `handleSetDefault`                                                                                                                                                                                            |
...
| `useWorkspaces`    | `composables/useWorkspaces.js`    | `workspaces`, `currentWorkspace`, `displayedWorkspaces` (filtered + sorted), `sortKey`, `loading`, `isModalVisible`, `editingWorkspace`, `isEditing`, `nameRules`, `openCreateModal`, `openEditModal`, `closeModal`, `handleSubmit`, `handleDelete`, `fetchWorkspaces`, `fetchWorkspaceById`                                  |
...
| `useSuggestedPrompts` | `composables/useSuggestedPrompts.js` | `prompts` — reactive array of `{ text }` objects (max 4) fetched from the first linked dataset's exploration questions. Watches the first `datasetIds` entry; clears on no selection; guards out-of-order responses via a request token; degrades to empty on error.                                                       |
| `useMarkdown`      | `composables/useMarkdown.js`      | `render(content, citationNumbers)` — chat markdown → DOMPurify-sanitized HTML; custom `[N]` citation extension renders `<span class="cite-ref">`. Optional `citationNumbers` gates chips: `null` (default) chips every marker, an array chips only those numbers (others fall back to literal text), `[]` suppresses all chips. `renderChunk(content)` — document-chunk markdown via an isolated `Marked` instance (no citation extension, so literal `[1]` stays text), also DOMPurify-sanitized. External links get `target="_blank" rel="noopener noreferrer"`.                                                                                                                                    |
...
| `useFormattedTime` | `composables/useFormattedTime.js` | All `utils/time` formatters (`relativeTime`, `shortDate`, `calendarDate`, `clockTime`, `timeOfDay`, `hourOfDay`, `absoluteDateTime`, `dayKey`, `calendarDaysAgo`) pre-bound to the signed-in user's saved timezone (reactive; falls back to UTC), plus the `timeZone` computed. The single source of truth for rendering absolute dates/times in the user's preferred zone. |
...
| `AppSidebar`           | `components/AppSidebar.vue`                    | Workspace-scoped navigation menu — reads `:workspaceId` from the route, lists conversations, permission-gated links, theme toggle. Includes an Invitations nav link to `/invitations`; the pending-count badge was removed. The view always renders empty because `fetchMyInvitations` is a no-op stub (backend list endpoint not yet wired). |
...
| `InviteFormModal`      | `components/InviteFormModal.vue`               | Invite member modal — email address field and role selection                                                                                                                                                                         |
...
| `auditMaps.js`         | `components/audit/auditMaps.js`                | Pure display mappers (`category`, `entityIcon`, `verb`, `resourceLabel`, `CATEGORIES`) translating the lean audit API shape into UI labels (timestamp formatting now lives in `utils/time.js` via `useFormattedTime`)                |
| `AgentDrawer`          | `components/chat/AgentDrawer.vue`              | Agent picker drawer for chat — selects which agent answers in the conversation                                                                                                                                                       |
...
| `promptIcons.js`       | `components/chat/promptIcons.js`               | `promptIcon(icon)` — maps starter-prompt icon keys to Lucide icon components (defaults to `MessageSquare`)                                                                                                                           |
...
| `permissionGroupIcons.js` | `components/roles/permissionGroupIcons.js`  | `permissionGroupIcon(key)` — maps permission-group keys (`workspace`, `roles`, `members`, `audit`, `datasets`, `files`, `agents`, `conversations`) to Lucide icon components (defaults to `LayoutGrid`)                             |
...
| datasets      | `api/datasets.js`      | `listDatasets`, `getDataset`, `createDataset`, `updateDataset`, `deleteDataset`, `listDatasetQuestions`       |
...
| `utils/storage.js`           | `getUserData`, `setUserData`, `clearUserData`, `ONBOARDING_KEY`, `clearOnboardingData`                                                |
| `utils/time.js`              | `relativeTime`, `shortDate`, `calendarDate`, `clockTime`, `timeOfDay`, `hourOfDay`, `absoluteDateTime`, `dayKey`, `calendarDaysAgo` — timezone-aware date/time formatters. Each absolute formatter takes an optional `{ timeZone }` (IANA zone) and falls back to UTC for empty/invalid zones; relative labels ("Xm ago") are zone-independent. Consume via the `useFormattedTime` composable to bind the signed-in user's saved timezone automatically. |
