...
<template>
  <div class="rail">
    ...
    <nav class="rail-nav" v-if="workspaceId">
      ...
      >
        <svg class="nav-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7">
    ...
    <nav class="rail-nav">
      ...
      >
        <svg class="nav-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7">
          ...
          <path d="M8 1.5v1M8 13.5v1M1.5 8h1M13.5 8h1M3.34 3.34l.71.71M11.95 11.95l.71.71M3.34 12.66l.71-.71M11.95 4.05l.71-.71" stroke-linecap="round"/>
        </svg>
        Settings
      </button>
    ...
    <div class="rail-footer">
      ...
      <button class="icon-btn" :title="theme === 'dark' ? 'Light mode' : 'Dark mode'" @click="toggleTheme">
        <svg v-if="theme === 'dark'" width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7">
          <circle cx="8" cy="8" r="3.5"/>
          <path d="M8 1v1.5M8 13.5V15M1 8h1.5M13.5 8H15M3.2 3.2l1.1 1.1M11.7 11.7l1.1 1.1M3.2 12.8l1.1-1.1M11.7 4.3l1.1-1.1" stroke-linecap="round"/>
        </svg>
        <svg v-else width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7">
      ...
      <button class="icon-btn" title="Sign out" @click="handleLogout">
        <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.7">
          <path d="M9 2h3a1 1 0 011 1v8a1 1 0 01-1 1H9M5 9.5L2 7l3-2.5M2 7h8" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </button>
    </div>
  </div>
...
.ws-pill__name { font-size: 12.5px; font-weight: 500; color: var(--ink-2); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.ws-pill__caret { color: var(--ink-4); flex-shrink: 0; }
...
.nav-item:hover { background: rgba(24,18,12,0.05); color: var(--ink); }
.nav-item.active {
  background: var(--surface);
  color: var(--ink);
  font-weight: 600;
...
.nav-icon { width: 15px; height: 15px; flex-shrink: 0; color: inherit; }
.nav-badge {
...
.rail-divider { height: 1px; background: var(--line); margin: 8px 14px; flex-shrink: 0; }
...
.user-avatar {
  ...
  font-size: var(--t-xs); font-weight: 700; flex-shrink: 0;
}
.user-info { flex: 1; min-width: 0; }
.user-name { font-size: var(--t-sm); font-weight: 600; color: var(--ink); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.user-role { font-size: var(--t-xs); color: var(--ink-4); margin-top: 1px; }
.icon-btn {
  display: flex; align-items: center; justify-content: center;
  width: 28px; height: 28px;
  border: none; background: none; cursor: pointer;
  color: var(--ink-4); border-radius: var(--r-sm); flex-shrink: 0;
