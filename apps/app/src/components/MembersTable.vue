...
<style scoped>
.members-wrap { background: var(--surface); border: 1px solid var(--line); border-radius: var(--r); overflow: hidden; box-shadow: var(--shadow-1); }
.filter-row { display: flex; align-items: center; gap: 8px; padding: 12px 16px; border-bottom: 1px solid var(--line); }
.filter-row :deep(.ant-input-affix-wrapper) { background: var(--surface); border-color: var(--line-2); border-radius: var(--r-sm); font-size: 13px; }
.filter-row :deep(.ant-input-affix-wrapper:focus-within) { border-color: var(--brand); box-shadow: 0 0 0 3px rgba(255,107,53,0.10); }
.member-cell { display: flex; align-items: center; gap: 10px; }
.avatar { width: 28px; height: 28px; border-radius: 50%; background: var(--brand-tint); color: var(--brand-3); font-weight: 600; font-size: 10.5px; display: grid; place-items: center; flex-shrink: 0; }
.member-name { font-size: 13px; font-weight: 500; color: var(--ink); }
.member-email { font-size: 11.5px; color: var(--ink-4); }
