...
<style scoped>
:deep(.ant-modal-content) { border-radius: var(--radius-lg); }
:deep(.ant-modal-header) { border-radius: var(--radius-lg) var(--radius-lg) 0 0; padding: 20px 24px 16px; border-bottom: 1px solid var(--color-border); }
:deep(.ant-modal-title) { font-size: var(--text-md); font-weight: 700; color: var(--color-text-primary); }
:deep(.ant-modal-body) { padding: 20px 24px 24px; }
:deep(.ant-form-item-label > label) { font-size: var(--text-sm); font-weight: 500; color: var(--color-text-secondary); }
:deep(.ant-input), :deep(.ant-input-textarea textarea), :deep(.ant-select-selector), :deep(.ant-input-number) { background: var(--color-bg) !important; border-color: var(--color-border) !important; border-radius: var(--radius-sm) !important; font-size: var(--text-base) !important; }
:deep(.ant-input:focus), :deep(.ant-input-focused), :deep(.ant-select-focused .ant-select-selector) { border-color: var(--color-accent) !important; box-shadow: 0 0 0 3px rgba(29,78,216,0.1) !important; }
:deep(.ant-btn-primary) { background: var(--color-accent); border-color: var(--color-accent); border-radius: var(--radius-sm); height: 40px; font-weight: 600; }
:deep(.ant-btn-primary:hover) { background: var(--color-accent-hover); border-color: var(--color-accent-hover); }
</style>
