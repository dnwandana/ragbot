  ...
  <AuthShell>
    <div class="auth-card">
      <template v-if="sent">
        ...
        <div class="auth-subtitle">If that email is registered, a reset link is on its way. It expires in 1 hour.</div>
      ...
      <template v-else>
        ...
        <a-form layout="vertical" @finish="handleSubmit">
          <a-form-item name="email" :rules="[{ required: true, type: 'email', message: 'Please enter a valid email' }]">
...
.back-link { display: inline-block; margin-top: 20px; font-size: 13px; color: var(--brand); font-weight: 500; text-decoration: none; }
...
:deep(.ant-input) { background: var(--surface); border-color: var(--line-2); border-radius: var(--r-sm); color: var(--ink); }
:deep(.ant-input:focus) { border-color: var(--brand); box-shadow: 0 0 0 3px rgba(255,107,53,0.12); }
