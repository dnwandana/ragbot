...
.field-label-row { display: flex; align-items: center; justify-content: space-between; width: 100%; }
.forgot-link { font-size: 12.5px; font-weight: 500; color: var(--brand); text-decoration: none; }
.forgot-link:hover { color: var(--brand-2); }
.form-error {
  font-size: 13px; color: var(--err); background: var(--err-bg);
  border: 1px solid var(--err-border); border-radius: var(--r-sm);
  padding: 10px 12px; margin-bottom: 14px;
}
.btn-brand {
  width: 100%; padding: 10px 16px; margin-top: 4px; margin-bottom: 20px;
...
:deep(.ant-input:focus), :deep(.ant-input-affix-wrapper-focused) {
  border-color: var(--brand); box-shadow: 0 0 0 3px rgba(255,107,53,0.12);
}
