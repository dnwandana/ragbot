          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.key === 'status'">
              <span class="chip" :class="`chip--${statusVariant(record.status)}`">
                <span
                  v-if="['processing', 'queued'].includes(record.status)"
                  class="status-dot status-dot--pulse"
                />
...
.file-shimmer {
  height: 3px;
  background: linear-gradient(90deg, var(--brand) 0%, var(--brand-tint) 50%, var(--brand) 100%);
