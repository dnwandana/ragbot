...
const { conversations, loading, isModalVisible, openCreateModal, closeModal, handleCreate, handleDelete, fetchConversations } = useConversations(workspaceId)
...
onMounted(async () => {
  await Promise.all([
    fetchConversations(),
    agentsStore.fetchAgents(workspaceId).then(() => { agents.value = agentsStore.agents }),
    datasetsStore.fetchDatasets(workspaceId).then(() => { datasets.value = datasetsStore.datasets }),
  ])
...
<template>
  <div class="page">
    ...
    <!-- Loading skeleton -->
    <a-skeleton v-if="loading && !conversations.length" active :paragraph="{ rows: 4 }" style="padding: 24px" />
    ...
    <div v-else class="conv-list">
      <template v-for="group in grouped" :key="group.label">
        ...
        >
          <div class="conv-icon">
            <svg width="17" height="17" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7">
              <path d="M2 2h12a1 1 0 011 1v8a1 1 0 01-1 1H5l-3 2V3a1 1 0 011-1z" stroke-linejoin="round"/>
            </svg>
          </div>
          <div class="conv-body">
            ...
            <div class="conv-meta">
              ...
              <span class="chip-sm">{{ (conv.dataset_ids || []).length }} dataset{{ (conv.dataset_ids || []).length !== 1 ? "s" : "" }}</span>
            </div>
          </div>
    ...
    <a-modal :open="isModalVisible" title="New Conversation" @cancel="closeModal" :footer="null" :width="480">
      <a-form :model="form" layout="vertical" @finish="handleCreate(form)">
        <a-form-item label="Agent" name="agent_id" :rules="[{ required: true, message: 'Please select an agent' }]">
        ...
        <a-form-item label="Datasets (optional)">
          <a-select v-model:value="form.dataset_ids" mode="multiple" placeholder="Select datasets to search">
...
.btn-block { width: 100%; padding: 10px; justify-content: center; margin-top: 4px; }

.group-label { font-size: 10.5px; font-weight: 600; letter-spacing: 0.06em; text-transform: uppercase; color: var(--ink-4); padding: 12px 0 6px; }
...
.conv-row:hover { box-shadow: var(--shadow-2); border-color: var(--line-2); }
...
.conv-title { font-size: 13.5px; font-weight: 500; color: var(--ink); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 3px; }
.conv-meta { display: flex; align-items: center; gap: 8px; }
.conv-agent { font-size: 12px; color: var(--ink-3); }
...
.conv-time { font-size: 11.5px; color: var(--ink-4); flex-shrink: 0; }
.conv-more { display: flex; align-items: center; justify-content: center; width: 28px; height: 28px; border: none; background: transparent; cursor: pointer; color: var(--ink-4); border-radius: var(--r-sm); flex-shrink: 0; }
.conv-more:hover { background: var(--err-bg); color: var(--err); }
...
.conv-new:hover { border-color: var(--brand); color: var(--brand); background: var(--brand-tint); }
