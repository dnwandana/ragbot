...
const { agents, loading, isModalVisible, editingAgent, openCreateModal, openEditModal, closeModal, handleSubmit, handleDelete, fetchAgents } = useAgents(workspaceId)
  ...
  <div class="page">
    ...
    <div v-else-if="viewMode === 'cards'" class="agent-grid">
      <div v-for="agent in agents" :key="agent.id" class="agent-card">
        ...
        <p class="agent-desc">{{ (agent.description || agent.system_prompt || "").slice(0, 140) }}{{ (agent.description || agent.system_prompt || "").length > 140 ? "…" : "" }}</p>
        <div class="agent-card__foot">
          <button class="btn-outline btn-sm" @click="openEditModal(agent)">Edit</button>
          <button class="btn-danger-sm" :disabled="agent.is_system" @click="handleDelete(agent.id)">Delete</button>
        </div>
      </div>
    </div>
    ...
    <div v-else class="agent-table-wrap">
      <a-table :dataSource="agents" :loading="loading" :columns="tableColumns" :pagination="false" rowKey="id" size="middle" :bordered="false">
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'name'">
            <div>
              <div class="tbl-name">
                {{ record.name }}
                <span v-if="record.is_system" class="chip chip--ghost" style="margin-left:6px">System</span>
              </div>
          ...
          <template v-if="column.key === 'actions'">
            <div class="row-actions">
              ...
              <button class="btn-danger-sm" :disabled="record.is_system" @click="handleDelete(record.id)">Delete</button>
            </div>
          </template>
        </template>
      </a-table>
    ...
    <AgentFormModal :visible="isModalVisible" :agent="editingAgent" @close="closeModal" @submit="handleSubmit" />
  </div>
</template>
...
.btn-outline { display: inline-flex; align-items: center; padding: 7px 12px; background: var(--surface); color: var(--ink-2); border: 1px solid var(--line-2); border-radius: var(--r-sm); font-size: 13px; cursor: pointer; }
.btn-outline:hover { border-color: var(--brand); color: var(--brand); }
.btn-sm { padding: 5px 10px; font-size: 12px; }
.btn-xs { padding: 4px 10px; font-size: 12px; }
.btn-danger-sm { display: inline-flex; align-items: center; padding: 5px 10px; font-size: 12px; background: transparent; color: var(--err); border: 1px solid var(--err-border); border-radius: var(--r-sm); cursor: pointer; }
...
.agent-card { background: var(--surface); border: 1px solid var(--line); border-radius: var(--r); box-shadow: var(--shadow-1); display: flex; flex-direction: column; padding: 16px; gap: 10px; }
.agent-card__hd { display: flex; align-items: flex-start; gap: 12px; }
.agent-icon { width: 36px; height: 36px; border-radius: 9px; background: var(--brand-tint); color: var(--brand-3); display: grid; place-items: center; flex-shrink: 0; }
.agent-name { font-size: 14px; font-weight: 600; color: var(--ink); margin-bottom: 4px; display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
.agent-badges { display: flex; gap: 5px; flex-wrap: wrap; }
.agent-desc { font-size: 12.5px; color: var(--ink-3); line-height: 1.5; margin: 0; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; flex: 1; }
.agent-card__foot { display: flex; gap: 6px; padding-top: 6px; border-top: 1px solid var(--line); }
...
.model-chip { font-family: 'Geist Mono', monospace; font-size: 10.5px; }
.status-dot { width: 6px; height: 6px; border-radius: 50%; background: currentColor; }

.agent-table-wrap { background: var(--surface); border: 1px solid var(--line); border-radius: var(--r); overflow: hidden; box-shadow: var(--shadow-1); }
.tbl-name { font-size: 13.5px; font-weight: 500; color: var(--ink); display: flex; align-items: center; }
.tbl-desc { font-size: 12px; color: var(--ink-4); margin-top: 2px; }
.tbl-muted { font-size: 12.5px; color: var(--ink-4); font-family: 'Geist Mono', monospace; }
